...
class ObjectDetector(BaseModel):
        ...
        for i in range(3):
            for j in range(3):
                num_anchor, anchors, anchor_is_untruncated, num_untruncated_anchor, parent_anchors, parent_anchor_is_untruncated, num_untruncated_parent_anchor = generate_anchors(
                    img_shape, current_feat_shape, self.anchor_scales[i], self.anchor_ratios[j])
                if i == 0 and j == 0:
                    ...
                    self.parent_anchor_is_untruncated = np.concatenate(
                        (self.parent_anchor_is_untruncated, parent_anchor_is_untruncated))
    ...
    def get_feed_dict_for_rpn(self, batch, is_train, feats):
        ...
        if is_train:
            ...
            gt_anchor_labels, gt_anchor_regs, anchor_masks, anchor_weights, anchor_reg_masks = self.process_anchor_data(
                anchor_files)
            return {self.feats: feats, self.gt_anchor_labels: gt_anchor_labels, self.gt_anchor_regs: gt_anchor_regs,
                    self.anchor_masks: anchor_masks, self.anchor_weights: anchor_weights,
                    self.anchor_reg_masks: anchor_reg_masks, self.is_train: is_train}
    ...
    def get_feed_dict_for_rcn(self, batch, is_train, feats, rois=None, masks=None):
        ...
        if is_train:
            _, anchor_files = batch
            rois, gt_roi_classes, gt_roi_regs, roi_masks, roi_weights, roi_reg_masks = self.process_roi_data(
                anchor_files)
            ...
            return {self.roi_warped_feats: roi_warped_feats, self.gt_roi_classes: gt_roi_classes,
                    self.gt_roi_regs: gt_roi_regs, self.roi_masks: roi_masks, self.roi_weights: roi_weights,
                    self.roi_reg_masks: roi_reg_masks, self.is_train: is_train}
    ...
    def get_feed_dict_for_all(self, batch, is_train, feats=None):
        ...
        if is_train:
            ...
            return {self.feats: feats, self.gt_anchor_labels: gt_anchor_labels, self.gt_anchor_regs: gt_anchor_regs,
                    self.anchor_masks: anchor_masks, self.anchor_weights: anchor_weights,
                    self.anchor_reg_masks: anchor_reg_masks, self.roi_warped_feats: roi_warped_feats,
                    self.gt_roi_classes: gt_roi_classes, self.gt_roi_regs: gt_roi_regs, self.roi_masks: roi_masks,
                    self.roi_weights: roi_weights, self.roi_reg_masks: roi_reg_masks, self.is_train: is_train}
        ...
        for i in range(self.batch_size):
            ...
            for j in range(t):
                ...
                for k in range(self.num_anchor[j]):
                    ...
                    current_reg_masks[k] = flags[k] * self.anchor_iou_weight[j, idx1[k], idx2[k]] * (
                    current_labels[k] == 1)
    ...
    def process_roi_data(self, anchor_files):
        ...
        for i in range(self.batch_size):
            ...
            for k in range(num_real_roi):
                ...
                real_roi_reg_masks[k] = self.class_iou_weight[real_roi_classes[k], idx1[k], idx2[k]] * (
                real_roi_classes[k] != self.background_id)
    ...
    def prepare_anchor_data(self, dataset, show_data=False):
        ...
        for i in range(dataset.count):
            ...
            labels, bboxes, classes, ious, ioas, iogs = label_anchors(self.anchors, self.anchor_is_untruncated,
                                                                      gt_classes, gt_bboxes, self.background_id)
            ...
            np.savez(dataset.anchor_files[i], labels=labels, bboxes=bboxes, regs=regs, classes=classes, ious=ious,
                     ioas=ioas, iogs=iogs, sorted_idx=sorted_idx)
            ...
            if show_data:
                ...
                for k in range(self.total_num_anchor):
                    ...
                    if labels[k] == 1:
                        cv2.rectangle(img, (scaled_anchors[k][1], scaled_anchors[k][0]), (
                        scaled_anchors[k][1] + scaled_anchors[k][3] - 1,
                        scaled_anchors[k][0] + scaled_anchors[k][2] - 1), (255, 255, 255), 2)
