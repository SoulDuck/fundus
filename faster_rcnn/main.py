...
def main(argv):
    ...
    parser.add_argument('--mean_file', default='../dataset/pascal/ilsvrc_2012_mean.npy',
    ...
    parser.add_argument('--train_coco_annotation_file', default='./train/coco/instances_train2014.json',
                        help='JSON file storing the objects for COCO train2014 images')
    parser.add_argument('--train_coco_data_dir', default='./train/coco/data/',
                        help='Directory to store temporary training data for COCO')
    ...
    parser.add_argument('--val_coco_annotation_file', default='./val/coco/instances_val2014.json',
                        help='JSON file storing the objects for COCO val2014 images')
    """


    parser.add_argument('--train_pascal_image_dir', default='./dataset/pascal/images/',
                        help='Directory containing the PASCAL training images')
    parser.add_argument('--train_pascal_annotation_dir', default='../dataset/pascal/Annotations',
                        help='Directory containing the PASCAL training Annotations')
    parser.add_argument('--train_pascal_data_dir', default='./dataset/pascal/Data/',
                        help='Directory to store temporary training data for PASCAL')

    parser.add_argument('--val_pascal_image_dir', default='./val/pascal/images/',
                        help='Directory containing the PASCAL validation images')
    parser.add_argument('--val_pascal_annotation_dir', default='./val/pascal/annotations/',
                        help='Directory containing the PASCAL validation annotations')
    ...
    parser.add_argument('--bbox_per_class', action='store_true', default=False,
                        help='Turn on to do one bounding box regression for each class')
    ...
    parser.add_argument('--rpn_reg_weight', type=float, default=10.0,
                        help='Relative weight for bounding box regression loss vs classification loss of RPN')

    parser.add_argument('--rcn_reg_weight', type=float, default=10.0,
                        help='Relative weight for bounding box regression loss vs classification loss of RCN')
    parser.add_argument('--class_balancing_factor', type=float, default=0.8,
                        help='Class balancing factor. The larger it is, the more attention the rare classes receive.')
    parser.add_argument('--prepare_anchor_data', action='store_true', default=True,
                        help='Turn on to prepare useful anchor data for training. Must do this for the first time of training.')
    ...
    with tf.Session() as sess:
        # Training phaseƒ
        if args.phase == 'train':
            train_data = prepare_train_pascal_data(args) # new Class Dataset
            model = ObjectDetector(args, 'train')
            init = tf.group(tf.global_variables_initializer() , tf.local_variables_initializer())
            sess.run(init)
            if args.load: #default False
                model.load(sess)
            elif args.load_basic_model: #default False
                print 'no pretrained model'
                model.load2(args.basic_model_file, sess)
            print args.prepare_anchor_data
