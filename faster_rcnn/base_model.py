    ...
    def train_rpn(self, sess, train_dataset):
        ...
        for epoch_no in tqdm(list(range(params.num_epoch)), desc='epoch'):
            for idx in tqdm(list(range(train_dataset.num_batches)), desc='batch'):
                ...
                _, loss0, loss1, global_step = sess.run(
                    [self.rpn_opt_op, self.rpn_loss0, self.rpn_loss1, self.global_step], feed_dict=feed_dict)
    ...
    def train_rcn(self, sess, train_dataset):
        ...
        for epoch_no in tqdm(list(range(params.num_epoch)), desc='epoch'):
            for idx in tqdm(list(range(train_dataset.num_batches)), desc='batch'):
                ...
                _, loss0, loss1, global_step = sess.run(
                    [self.rcn_opt_op, self.rcn_loss0, self.rcn_loss1, self.global_step], feed_dict=feed_dict)
                print(" loss0=%f loss1=%f" % (loss0, loss1))
    ...
    def train(self, sess, train_dataset):
        ...
        for epoch_no in tqdm(list(range(params.num_epoch)), desc='epoch'):
            for idx in tqdm(list(range(train_dataset.num_batches)), desc='batch'):
                ...
                _, loss0, loss1, global_step = sess.run([self.opt_op, self.loss0, self.loss1, self.global_step],
                                                        feed_dict=feed_dict)
                print(" loss0=%f loss1=%f" % (loss0, loss1))
        ...
        for i in range(val_dataset.count):
            ...
            for s, c, b in zip(det_scores[i], det_classes[i], det_bboxes[i]):
                results[file_name].append(
                    {'class_id': c, 'bbox': [b[1], b[0], b[1] + b[3] - 1, b[0] + b[2] - 1], 'score': s})
        ...
        for i in range(test_dataset.count):
            ...
            for s, c, b in zip(det_scores[i], det_classes[i], det_bboxes[i]):
                results[img_file].append(
                    {'class_name': self.class_names[c], 'bbox': [b[1], b[0], b[3] - 1, b[2] - 1], 'score': s})
