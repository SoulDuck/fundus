...
def bagging_train(limit_n_normal , bagging_iter , max_iter , learning_rate, restored_model_folder_path ,check_point=100,nx=[10,10,10,5,5,5,35], structure='inception_A', optimizer='AdamOptimizer', restored_path_folder_path=None):
    train_list_imgs_labs, test_list_imgs_labs, train_list_file_paths, test_list_file_paths, names = data.fundus_300x300(reload_folder_path=restored_path_folder_path)
    for i,name in enumerate(names):
        indices=random.sample(range(len(normal_imgs_labs)) , limit_n_normal )
        if name == 'normal':
            normal_imgs_labs=train_list_imgs_labs[i]
            normal_imgs , normal_labs=normal_imgs_labs
            normal_imgs=normal_imgs[indices]
            normal_labs=normal_labs[indices]
            normal_imgs_labs=[normal_imgs , normal_labs ]
            train_list_imgs_labs[i] = normal_imgs_labs


    if restored_model_folder_path is None:
        restored_model_folder_path = utils.make_folder('./cnn_model/', 'bagging/')
    if restored_path_folder_path is None:
        restored_path_folder_path= utils.make_folder('./paths/' , 'bagging/')
    graph_saved_folder_path = utils.make_folder('./graph/', 'baggging/')
    log_saved_folder_path = utils.make_folder('./log/', 'baggging/')
    log_saved_file_path = log_saved_folder_path + 'log.txt'
    f = open(log_saved_file_path , 'w+' , 0)
...
def  bagging_train(src_paths_folder , n_bagging  , n_target  , save_paths):


    """
    have to read

    first , this function need src_paths_folder that included glaucoma_test_paths.txt , glaucoma_train_paths.txt ,normal,...
    and get paths from text file and divide normal paths by n_target_paths

    second
    make folder ./paths/bagging/0/0/ , ./paths/bagging/0/1/ , ./paths/bagging/0/2/ , ./paths/bagging/0/3 /.... ./paths/bagging/0/n_iter/
    so save_path has to be './paths/bagging/0' or './paths/bagging/0'

    in this funcution , extract data from normal data

    not yet implement 1. IOError control
    :param restored_path_folder_path:
    :return:
    """

    ret_list=[]
    utils.make_dir(save_paths)
    dir_path , _ , files=os.walk(src_paths_folder).next()
    for file in files:
    ...
    for i in range(n_bagging):
        tmp_paths=[]
        indices=random.sample( range(src_paths) , n_target_paths )
        for i,ind in enumerate(indices):
            tmp_paths.append(src_paths[ind])
            if save_paths is not None:
                utils.save_paths(tmp_paths , 'normal_'+str(i) )


if __name__ == '__main__':
    restored_path_folder_path='./paths/fundus/2'
    bagging_train(restored_path_folder_path)
